(function () {
  ...
  _.uniq = function (array, isSorted, iterator) {
    ...
    if (iterator) {
      newArr = [];
      let noDup1 = iterator(noDuplicate[0]);
      for (let i = 1; i < noDuplicate.length; i++) {
        if (iterator(noDuplicate[i]) !== noDup1) {
          newArr.push(noDuplicate[0], noDuplicate[i]);
          break;
        }
      }
    }
